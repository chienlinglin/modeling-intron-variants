...
all$novel.maxent[is.na(all$novel.maxent)]<- mean(all$novel.maxent, na.rm=T)
...
all$same_bp[is.na(all$same_bp)] <- "FALSE"
...
all1$wt_splicing_efficiency2[which(all1$wt_splicing_efficiency2==1)] <- 0.999 ## log(0.99/0.01)=4.59512; log(0.999/0.001)=6.906755
all1$wt_splicing_efficiency2[which(all1$wt_splicing_efficiency2==0)] <- 0.001
all1$wt_splicing_efficiency3 <- log(all1$wt_splicing_efficiency2/(1-all1$wt_splicing_efficiency2))

which(all1$wt_splicing_efficiency2== 0.999) #0
which(all1$wt_splicing_efficiency2== 0.001) #0
hist(all1$wt_splicing_efficiency2)
hist(all1$wt_splicing_efficiency3)

#################################################################################
### standardized wt model 

# wt have no novel 3'ss related factors
remove <- c(which(str_detect(colnames(all1),pattern = "novel")))
...
### Remove RBPs 
remove3 <- c(105:313,419:508)

allf <- all1[,-unique(c(remove,remove1,remove2,remove3))]
...
eff_model <- model.matrix(object = wt_splicing_efficiency3 ~ ., data = allf )[, -1]
...
eff_train_y<-as.numeric(allf$wt_splicing_efficiency3[sel.train])
eff_test_y<-as.numeric(allf$wt_splicing_efficiency3[sel.test])
dim(eff_test_x) #1380 127
dim(eff_train_x) #2761 127
length(eff_test_y)
length(eff_train_y)
...
coef(cv_lasso, s = "lambda.min") %>% 
  ...
  geom_point() +
  ggtitle("Top 15 influential variables in wt splicing efficiency") +
  xlab("Coefficient") +
  ylab(NULL)
dev.off()
...
pear_cor <- round(cor(final$predicted, final$eff_test_y), 3) #0.484
rmselabel <- paste0("R square= 0.23", "\n RMSE = 0.64")
p <- ggplot(data=final, aes(x=predicted, y=eff_test_y)) + geom_point(colour = "gray") + ggtitle("wt_eff: real vs prediction") + geom_smooth(method=lm, se=T,colour="peachpuff4")+
  theme_light()+annotate(geom = "text",label=rmselabel,x=1.8,y=4.8,colour="lightsalmon",fontface = "bold") + annotate(geom = "text",label="Pearson's p = 0.48",x=1.9,y=-0.5,colour="peachpuff4",fontface = "bold")
ggsave("wt_standardized_efficiency_performance.pdf", width = 4, height = 4)

#################################################################################
### non-standardized wt model for server 
### Remove RBPs motifs appears <10 times across all wt sequences
...
# wt have no mt related factors
remove4 <- which(str_detect(colnames(all1),pattern = "delta"))
remove5 <- which(str_detect(colnames(all1),pattern = ".mt"))

allf <- all1[,-c(1:22,24,26,27:50,51,62,66:86,94:96,98,105:314,316,317,319,320,509,510,512:514,516:518,520:522,524:530,532,533,535:539,540,543:553,555:557,remove,remove2,remove3,remove4,remove5)]

which(is.na(allf), arr.ind=TRUE)
allf <- na.omit(allf)
allf <- allf[,-(15:74)] ### remove all RBPs
...
eff_train_y<-as.numeric(allf$wt_splicing_efficiency3[sel.train])
eff_test_y<-as.numeric(allf$wt_splicing_efficiency3[sel.test])
dim(eff_test_x) #1380 29
dim(eff_train_x) #2759 29
